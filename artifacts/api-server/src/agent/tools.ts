...
const ON_TOPIC_KEYWORDS = [
  ...
  // quantum physics (considered part of cosmology by the user)
...
// ── Subject extractor ──────────────────────────────────────────────────────────
function extractMainSubject(question: string): string {
  return question
...
export const searchKnowledgeBaseTool = tool(
  async ({ query }) => {
    ...
    return data
      .map((r: { title: string; summary?: string; content?: Record<string, string>; similarity: number }) => {
        // For agent-written docs content.text holds the full response — prefer it over summary.
        // For preloaded docs, join all content fields.
...
export const searchWebTool = tool(
  async ({ question }) => {
    ...
    // Use the extracted subject as the query — no year appended, Tavily handles recency natively
  ...
  },
  {
    ...
    description:
      ...
      "Use when the knowledge base has no relevant results or the user asks about recent events.",
    schema: z.object({
...
export const searchResearchPapersTool = tool(
  ...
  },
  {
    ...
    description:
      ...
      "Use for scientific depth, citations, or when the user asks for research or studies.",
    schema: z.object({
