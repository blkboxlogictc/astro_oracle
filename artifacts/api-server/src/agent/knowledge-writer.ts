...
function buildTitle(question: string): string {
  return question
    ...
    // Strip recency/temporal qualifiers so the title stays evergreen
    ...
    // Strip dangling year numbers
    .replace(/\b(19|20)\d{2}\b/g, "")
    // Strip trailing "launched", "discovered", "found" that become orphaned after stripping
...
export async function maybeWriteToKnowledgeBase(
  question: string,
  ...
  if (agentResponse.length < MIN_CONTENT_LENGTH)
    return { saved: false, reason: "response too short" };
  ...
  if (await isDuplicate(embedding))
    return { saved: false, reason: "duplicate content already in KB" };
